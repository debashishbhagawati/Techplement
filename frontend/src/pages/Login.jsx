...
const Login = () => {
  ...
  const {userName, setUserName, setUserEmail, setIsLoggedIn} = useContext(AppContext)
  ...
  const loginHandler = async (event) => {
    ...
    try {
      ...
      // Assuming a successful login returns a token or similar
      setUserName(name);
      setUserEmail(email);
      setIsLoggedIn(true); 
      console.log(name);
    ...
    <div className="w-screen h-screen flex justify-center items-center bg-gray-100">
      <div className="flex gap-8 bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="flex flex-col justify-between p-8 w-full max-w-md">
          ...
          <h1 className="font-bold text-2xl text-gray-900 mb-4">Login</h1>
          <form className="flex flex-col text-gray-700 space-y-4" onSubmit={loginHandler}>
          <input
              type="text"
              placeholder="Name"
              value={name}
